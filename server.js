...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    ...
    // Send email notification. This is awaited — in a serverless
    // environment, a "fire and forget" promise can get killed the
    // moment the response is sent, since Vercel may freeze/terminate
    // the function as soon as res.json() completes. Awaiting it (but
    // catching errors) ensures the email actually gets a chance to
    // send, without failing the form submission if email fails.
    try {
      await sendNotificationEmail({ name: cleanName, email: cleanEmail, service, message: cleanMessage });
    } catch (err) {
